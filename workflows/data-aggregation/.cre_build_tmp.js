...
function numberToBytesBE(n, len2) {
  anumber(len2);
  n = abignumber(n);
  const res = hexToBytes2(n.toString(16).padStart(len2 * 2, "0"));
  if (res.length !== len2)
...
function aggregateDemographics(records) {
  const uniquePatients = new Set(records.map((r) => r.patientAddress.toLowerCase()));
  ...
  for (const r of records) {
    templateTypeCounts[r.templateType] = (templateTypeCounts[r.templateType] ?? 0) + 1;
  }
  return {
    total_patients,
...
function aggregateConditions(records) {
  ...
  for (const r of records) {
    const clean3 = deidentify(r.formData);
    for (const field of diagnosisFields) {
      const val = clean3[field];
      if (val && val.trim()) {
  ...
  const sorted = Object.entries(anonymized).sort(([, a], [, b]) => b - a).slice(0, 10).map(([condition, count]) => ({
    condition,
    count,
...
function aggregateOutcomes(records) {
  const dischargeCounts = {};
  let complicationsCount = 0;
  for (const r of records) {
    const clean3 = deidentify(r.formData);
    const dischargeVal = clean3["dischargeCondition"] ?? clean3["dischargeDisposition"];
    ...
    if (clean3["complications"] && clean3["complications"].trim()) {
      complicationsCount++;
    }
  }
  const total_records = laplaceNoise(records.length);
  const complications_rate = records.length > 0 ? parseFloat((complicationsCount / records.length * 100).toFixed(1)) : 0;
  return {
    total_records,
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`data-aggregation: endpoint=${body.endpoint} queryId=${body.queryId}`);
  const privKeyB64 = runtime2.getSecret({ id: "CRE_MARKETPLACE_PRIVATE_KEY" }).result().value;
  ...
  const getContributors = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddr,
  ...
  const contributors = [];
  try {
    const hex = contributorsRaw.startsWith("0x") ? contributorsRaw.slice(2) : contributorsRaw;
    const arrayLen = parseInt(hex.slice(64, 128), 16);
    for (let i2 = 0;i2 < arrayLen; i2++) {
      const addrHex = hex.slice(128 + i2 * 64 + 24, 128 + i2 * 64 + 64);
      contributors.push(`0x${addrHex}`);
    }
  } catch {
    runtime2.log("Failed to decode contributors list");
  }
  runtime2.log(`Found ${contributors.length} active contributors`);
  const records = [];
  for (const patient of contributors) {
    const getKeySelector = "0x49d5fb72";
    const paddedAddr = patient.slice(2).padStart(64, "0");
    const getKey = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddr,
        to: runtime2.config.dataMarketplaceAddress,
        data: `0x${getKeySelector.slice(2)}${paddedAddr}`
      })
    });
    const keyRaw = bytesToHex(getKey.result().data);
    if (!keyRaw || keyRaw === "0x" || keyRaw.length < 10) {
      runtime2.log(`Patient ${patient}: no marketplace key — skipping`);
      continue;
    }
    let wrappedBundle;
    try {
      ...
      runtime2.log(`Patient ${patient}: failed to decode wrapped key — skipping`);
      continue;
    }
    let dataKey;
    try {
      ...
      runtime2.log(`Patient ${patient}: key unwrap failed — skipping`);
      continue;
    }
    const getRecordsSelector = "0x3aabf8b6";
    const getRecords = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddr,
        to: runtime2.config.healthRecordRegistryAddress,
        data: `0x${getRecordsSelector.slice(2)}${paddedAddr}`
      })
    });
    const recordsRaw = bytesToHex(getRecords.result().data);
    const recordIds = [];
    try {
      const hex = recordsRaw.startsWith("0x") ? recordsRaw.slice(2) : recordsRaw;
      const arrayLen = parseInt(hex.slice(64, 128), 16);
      for (let i2 = 0;i2 < arrayLen; i2++) {
        recordIds.push(parseInt(hex.slice(128 + i2 * 64, 128 + i2 * 64 + 64), 16));
      }
    } catch {
      runtime2.log(`Patient ${patient}: failed to decode record IDs`);
      continue;
    }
    for (const recordId of recordIds) {
      const getRecordSelector = "0x8f90e575";
      const paddedId = recordId.toString(16).padStart(64, "0");
      const getRecord = evmClient.callContract(runtime2, {
        call: encodeCallMsg({
          from: zeroAddr,
          to: runtime2.config.healthRecordRegistryAddress,
          data: `0x${getRecordSelector.slice(2)}${paddedId}`
        })
      });
      ...
      try {
        ...
        recordType2 = parseInt(hex.slice(64, 128), 16).toString();
      } catch {
        runtime2.log(`Record ${recordId}: failed to parse CID — skipping`);
        continue;
      }
      if (!ipfsCid)
        continue;
      const fetchBlob = httpClient.sendRequest(runtime2, (sender) => {
        const resp = sender.sendRequest({
          url: `${runtime2.config.pinataGatewayUrl}/ipfs/${ipfsCid}`,
          method: "GET",
      ...
      try {
        ...
        runtime2.log(`Record ${recordId}: decrypt/parse failed — skipping`);
        continue;
      }
    }
  }
  runtime2.log(`Aggregating ${records.length} decrypted records`);
  let result;
