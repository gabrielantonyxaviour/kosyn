...
function parseNillionResponse(raw) {
  try {
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  const claudeServiceUrl = runtime2.config.claudeServiceUrl;
  const claudeModel = runtime2.config.claudeModel;
...
Session summary: ${body.sessionSummary}`;
  ...
  runtime2.log("Attestation text generated inside TEE");
  ...
  runtime2.log(`Attestation stored on IPFS: ${ipfsCid}`);
  ...
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`Attestation recorded on-chain. TX: ${txHash}`);
  } else {
