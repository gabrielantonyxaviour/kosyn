...
var onLogTrigger = (runtime2, log) => {
  ...
  runtime2.log(`Processing marketplace query #${queryId} from ${requesterAddress}, payment: ${payment}`);
  ...
  const contributorsReply = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: "0x0000000000000000000000000000000000000000",
      to: runtime2.config.marketplaceAddress,
      data: getContributorsCallData
    })
  }).result();
  const contributorsHex = bytesToHex(contributorsReply.data);
  ...
  if (!contributors || contributors.length === 0) {
    runtime2.log("No active contributors. Fulfilling query with empty distribution.");
    const resultHash2 = keccak256(toHex(`queryId:${queryId}:no-contributors`));
    const innerEncoded2 = encodeAbiParameters(parseAbiParameters("uint256, bytes32, address[], uint256[]"), [queryId, resultHash2, [], []]);
    const encodedPayload2 = `0x05${innerEncoded2.slice(2)}`;
    const reportRequest2 = prepareReportRequest(encodedPayload2);
    const report3 = runtime2.report(reportRequest2).result();
  ...
  runtime2.log(`Distributing ${sharePerContributor} KUSD wei to each of ${contributors.length} contributors`);
  const resultHash = keccak256(toHex(JSON.stringify({
    queryId: queryId.toString(),
    contributors: contributors.map((c) => c.toLowerCase()),
    payment: payment.toString()
  })));
  const innerEncoded = encodeAbiParameters(parseAbiParameters("uint256, bytes32, address[], uint256[]"), [queryId, resultHash, contributors, shares]);
  const encodedPayload = `0x05${innerEncoded.slice(2)}`;
  ...
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`Query #${queryId} fulfilled. ${contributors.length} contributors paid. TX: ${txHash}`);
  } else {
...
var initWorkflow = (config) => {
  const querySubmittedTopic = keccak256(toHex("QuerySubmitted(uint256,address,uint256)"));
  const evmClient = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["avalanche-testnet-fuji"]);
