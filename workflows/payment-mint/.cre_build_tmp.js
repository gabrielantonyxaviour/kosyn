...
var onHttpTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`Processing payment: ${body.stripePaymentId}, amount: ${body.amount}, recipient: ${body.recipientAddress}`);
  const stripeKey = runtime2.getSecret({ id: "STRIPE_SECRET_KEY" }).result().value;
  const httpClient = new ClientCapability2;
  const doVerify = httpClient.sendRequest(runtime2, (sender) => {
  ...
  if (verifyParsed.statusCode === 200) {
    try {
      const paymentIntent = JSON.parse(verifyParsed.body);
      if (paymentIntent.status === "succeeded") {
        verifiedAmount = paymentIntent.amount / 100;
      }
      runtime2.log(`Stripe verification: status=${paymentIntent.status}, amount=${verifiedAmount} USD`);
    } catch {
      runtime2.log("Stripe response parse error");
    }
  } else {
    runtime2.log(`Stripe API returned ${verifyParsed.statusCode}`);
  }
  if (verifiedAmount <= 0) {
    runtime2.log("Payment verification failed. Aborting mint.");
    return {};
  }
  const kusdAmount = BigInt(Math.round(verifiedAmount * 1e6));
  const stripePaymentIdHash = keccak256(toHex(body.stripePaymentId));
  const innerEncoded = encodeAbiParameters(parseAbiParameters("address, uint256, bytes32"), [
    body.recipientAddress,
    kusdAmount,
    stripePaymentIdHash
  ]);
  const encodedPayload = `0x04${innerEncoded.slice(2)}`;
  ...
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`KUSD minted: ${kusdAmount} to ${body.recipientAddress}. TX: ${txHash}`);
  } else {
