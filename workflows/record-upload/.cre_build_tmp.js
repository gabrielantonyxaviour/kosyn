...
var onHttpTrigger = (runtime2, payload) => {
  ...
  if (!body.patientAddress || !body.encryptedData) {
    throw new Error(`Missing required fields: patientAddress=${body.patientAddress}, encryptedData=${!!body.encryptedData}`);
  }
  runtime2.log(`Processing record upload for patient: ${body.patientAddress}, type: ${body.recordType}`);
  ...
  const parsed = JSON.parse(ipfsResponseText);
  const ipfsCid = parsed.IpfsHash;
  runtime2.log(`Uploaded to IPFS: ${ipfsCid}`);
  const timestamp = BigInt(Math.floor(Date.now() / 1000));
  const innerEncoded = encodeAbiParameters(parseAbiParameters("string, uint8, uint256, address"), [ipfsCid, body.recordType, timestamp, body.patientAddress]);
  ...
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`Record uploaded on-chain. TX: ${txHash}`);
  } else {
